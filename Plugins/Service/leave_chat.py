...
@app.on_callback_query(filters.regex('^SERVICE_LAVE_CAHT$'))
async def SERVICE_LAVE_CAHT(app: app, query: types.CallbackQuery):
    _type = "خروج من قناة"
    ...
    for session in sessions:
        try:
            response = await chat_apis.LEAVE_CHAT(sessions[session]['session_string'], ChatUsername)
            if response == True:
                DONE+=1
            else:
                ERRO+=1
